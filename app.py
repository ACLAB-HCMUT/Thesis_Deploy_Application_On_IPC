...
from services.relay_services import service_check_and_update_relay
...
# Chỉ khởi tạo và chạy scheduler nếu chưa được khởi tạo
try:
    if not scheduler_initialized:
        scheduler.add_job(service_check_and_update_relay, 'interval', minutes=1)
        scheduler.start()
        print("APScheduler started, running service_check_and_update_relay every minute")
        scheduler_initialized = True
except Exception as e:
    print(f"Failed to start APScheduler: {str(e)}")

# Đảm bảo scheduler dừng khi ứng dụng kết thúc
import atexit
